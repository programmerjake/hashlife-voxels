...
#undef VULKAN_DRIVER_END
    ...
    struct GPUState final
    {
        ...
        const std::shared_ptr<VulkanFunctions> vk;
        const WMHelper *wmHelper;
        explicit GPUState(std::shared_ptr<Implementation> imp)
            : imp(std::move(imp)), vk(VulkanFunctions::loadVulkanLibrary())
        {
        }
    };
    struct TrackedPublicObject
    {
        std::shared_ptr<Implementation> imp;
        util::Spinlock gpuStateLock;
        explicit TrackedPublicObject(std::shared_ptr<Implementation> imp)
            : imp(std::move(imp)), gpuStateLock()
        {
        }
        virtual void attachGPUState(const std::shared_ptr<GPUState> &gpuState,
                                    std::unique_lock<util::Spinlock> &lock) = 0;
        virtual void detachGPUState(std::unique_lock<util::Spinlock> &lock) noexcept = 0;
        virtual ~TrackedPublicObject() = default;
    };
    class Buffer final
    {
        ...
        struct MappedBuffer final
        {
            ...
            ~MappedBuffer()
            {
                if(memory) // if map succeded
                {
                    ...
                    buffer->vk->UnmapMemory(*buffer->device, buffer->deviceMemory);
                }
            }
        };
        struct PrivateAccess final
    ...
    private:
                            ...
                            const VkPhysicalDeviceMemoryProperties &physicalDeviceMemoryProperties)
        {
            ...
            handleVulkanResult(vk->CreateBuffer(*device, &createInfo, nullptr, &buffer),
                               "vkCreateBuffer");
            try
            {
                ...
                handleVulkanResult(
                    vk->AllocateMemory(*device, &allocateInfo, nullptr, &deviceMemory),
                    "vkAllocateMemory");
                handleVulkanResult(vk->BindBufferMemory(*device, buffer, deviceMemory, 0),
                                   "vkBindBufferMemory");
            }
            catch(...)
            {
    ...
    public:
        Buffer(std::shared_ptr<const VkDevice> device,
               ...
               const VkPhysicalDeviceMemoryProperties &physicalDeviceMemoryProperties,
               std::shared_ptr<VulkanFunctions> vulkanFunctions,
               PrivateAccess)
        ...
        {
            allocateBuffer(usage, physicalDeviceMemoryProperties);
        }
        ...
        ~Buffer()
        {
            ...
            vk->DestroyBuffer(*device, buffer, nullptr);
            vk->FreeMemory(*device, deviceMemory, nullptr);
        }
    ...
    struct BaseBufferMemoryManager final
    {
        ...
        {
            return new std::shared_ptr<Buffer>(Buffer::make(
                imp->atomicDevice.get(), size, usage, imp->physicalDeviceMemoryProperties, vk));
        }
    };
    ...
    struct VulkanTextureImplementation final : public TextureImplementation,
                                               public TrackedPublicObject
    {
#warning finish
        const std::size_t width;
        const std::size_t height;
        VulkanTextureImplementation(std::shared_ptr<Implementation> imp,
                                    const std::shared_ptr<Image> &image)
            : TrackedPublicObject(std::move(imp)), width(image->width), height(image->height)
        {
        }
        virtual void attachGPUState(const std::shared_ptr<const VkDevice> &device) override
        {
        }
        virtual void detachGPUState() noexcept override
        {
        }
    };
    ...
    struct VulkanRenderBuffer final : public RenderBuffer, public TrackedPublicObject
    {
#warning finish
    ...
    struct VulkanCommandBuffer final : public CommandBuffer, public TrackedPublicObject
    {
        std::shared_ptr<const VkDevice> device;
        std::shared_ptr<const VkCommandPool> commandPool;
...
public:
    ...
    util::Spinlock trackedPublicObjectsLock;
    std::list<std::weak_ptr<TrackedPublicObject>> trackedPublicObjects;
    util::Spinlock texturesLock;
    std::list<std::shared_ptr<VulkanTextureImplementation>> textures;
...
public:
    void addTrackedPublicObjects(const std::shared_ptr<TrackedPublicObject> &object)
    {
        std::list<std::weak_ptr<TrackedPublicObject>> objectList;
        objectList.push_back(object);
        std::unique_lock<util::Spinlock> lockIt(trackedPublicObjectsLock);
        trackedPublicObjects.splice(trackedPublicObjects.end(), objectList);
        lockIt.unlock();
    }
...
{
    auto texture =
        std::make_shared<Implementation::VulkanTextureImplementation>(implementation, image);
    implementation->addTrackedPublicObjects(texture);
    std::list<std::shared_ptr<Implementation::VulkanTextureImplementation>> textureList;
    TextureId retval(texture.get());
    textureList.push_back(std::move(texture));
    std::unique_lock<util::Spinlock> lockIt(implementation->texturesLock);
    implementation->textures.splice(implementation->textures.end(), textureList);
    lockIt.unlock();
    return retval;
}
    ...
    const util::EnumArray<std::size_t, RenderLayer> &maximumSizes)
{
    ...
    implementation->addTrackedPublicObjects(retval);
    return retval;
}
...
{
#warning finish
    auto retval = std::make_shared<Implementation::VulkanCommandBuffer>(
        implementation->atomicDevice.get(), implementation);
    implementation->addTrackedPublicObjects(retval);
    return retval;
}
